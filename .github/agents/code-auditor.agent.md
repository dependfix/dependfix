...
- **commit message 信息密度（必查项）**：按 [git.md §3.6 信息密度规范](../../docs/standards/git.md) 核验：① 是否含 `pnpm ... 0 error` / `N passed` / `+N/-M` 等命令执行结果数字？→ warning；② 是否含"确切路径需源码进一步实证"等没实证的废话？→ warning；③ 是否含与本 commit 实际改动关联度低的"教训：..."段（教训应归属 hotfix 修复 commit 而非 docs 登记 commit）？→ warning；④ 关键决策 / 关联 commit 引用 / 关联 todo 条目是否保留？→ 必须保留。违规即退回精简（commit message 重写后再次提交审计）。**实证教训**——M22.7 hotfix `f617b56` + M22.8 hotfix `2472b05` 含验证命令 + 结果数字 + 改动行数等冗余信息，与 git diff / CI 实测输出重叠，无增量价值。
