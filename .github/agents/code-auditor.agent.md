...
- **docs 裸 HTML 标签与本地 docs:build 验证（必查项）**：改动 `docs/` 站点内 md（含 experience-archive.md 等持续追加文档）时，检查：① 正文/表格是否含裸 `<tag>` 占位符（`rg '<[a-z][a-z0-9-]*>'` 后人工过滤反引号内命中）或加粗内裸 `*`（如 `*.test.ts`），裸标签会在 docs build 报 `Element is missing end tag`（行号为转换产物行号）；② 是否已本地执行 `pnpm --filter dependfix-docs build` 并提供通过证据——lint:md 与 check:links 均不查 HTML 标签配对，docs build 是唯一防线。缺失即退回补验证（规范见 [documentation.md §2 裸 HTML 标签禁令](../../docs/standards/documentation.md)，教训见 [经验归档 §三十九](../../docs/design/governance/experience-archive.md)）。
...
- **批量替换转义残留（必查项）**：diff 疑似经 PowerShell 批量替换（`-replace`/`Replace`/`Set-Content`）或多文件机械替换时，检查：① 字面量转义残留——Node 字节扫描变更文件中的字面量 `\r?\n`（反斜杠形态）与"反引号 + n"字符序列（PowerShell 替换文本不做转义解释、单引号完全字面，2026-08-14 backlog.md 六处状态行损坏实证）；② 既有内容误伤——`git diff` 中非预期行（已知条目内容被拆行/截断/前缀丢失）逐条核验，`String.Replace` 短序列全局替换会拆坏"反引号 + n"（`npm_config_registry` → "换行 + pm_config_registry" 实证）；③ lint/check:links/docs:build 均不检测文本语义，内容级验证（Node 字节抽查 + diff 收敛行数）由调用方补证，缺失即退回补证（规范见 [ai-collaboration.md §1.2 执行原则 6](../../docs/standards/ai-collaboration.md)，检查点见 [code-quality-checklist 批量替换小节](../../.github/skills/code-reviewer/references/code-quality-checklist.md)，教训见 [经验归档 §四十](../../docs/design/governance/experience-archive.md)）。
