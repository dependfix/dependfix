...
### 步骤 2.5：按 audit-depth 分配审查深度（控制用时）⚠️ 必做

审查投入与改动风险匹配，不应对所有改动一视同仁长时间分析。`audit-depth` 分级（`quick` / `standard` / `deep`）、适用改动与时间盒以 [AI 协作规范 §1.3 分级审计执行协议](../../../docs/standards/ai-collaboration.md) 为唯一权威；调用方未声明时按 `deep` 防御执行。证据获取遵循"翻源码是最后手段"（优先级见同一节）。

执行规则：

- **证据优先采信**：任务方提供的已查证事实（实验证据、测试结果、源码行号引用）直接采用，翻源码仅限需要最终实锤且无外部参考的场景。
- **收敛策略（不依赖时间感知）**：审计方无真实时间感知，不检查时间、不因时间收敛——审查输出固定为"audit-depth 审查范围内可交付的结论 + 未覆盖边界"，宁可给 `Reject`（附待补证据清单）也不无限深挖；是否超时由调用方事后实测判定，不回溯要求补动作（见 [AI 协作规范 §1.3 真实用时实测](../../../docs/standards/ai-collaboration.md)）。
...
- **用时反馈（调用方事后实测）**：LLM 无真实时间感知，自报用时不可信——"实际用时 / 是否超时间盒"由调用方按 [AI 协作规范 §1.3 真实用时实测](../../../docs/standards/ai-collaboration.md) 用宿主系统时钟事后实测回填（发起审计时记录启动时间戳，返回后计算 elapsed），审计方不自报时长、审计过程中不检查时间；实测超时仅作分级校准数据，不回溯要求补动作。

### 步骤 3：收集并延续审查证据

- 默认把临时审查记录写入 `artifacts/review-gate/`（纳入 `.gitignore`），文件名 `<date>-<scope>.md`。
- 证据记录按 [证据模板](./references/evidence-template.md) 预填与延续：首轮创建，多轮 review 复用同一份记录，按"第 1 轮"、"第 2 轮"追加，保留未关闭问题编号（RG-B/W/S 系列）与复查结论。
...
- 未覆盖边界:
- 后续补跑计划:
- 实际用时 / 是否超时间盒:（调用方宿主时钟事后实测回填，见 [AI 协作规范 §1.3](../../../docs/standards/ai-collaboration.md)）
```
