...
- **用时反馈（调用方实测）**：LLM 无真实时间感知，自报用时不可信——"实际用时 / 是否超时间盒"由调用方按 [AI 协作规范 §1.3 真实用时实测](../../../docs/standards/ai-collaboration.md) 用宿主系统时钟实测回填（发起审计时记录启动时间戳，返回后计算 elapsed），审计方不自报时长；判断接近时间盒上限时可执行时间命令对比 prompt 携带的启动时间戳（秒级成本，仅收敛判断用），已超时则停止扩大审查面并声明"已超时，以下为已覆盖范围结论"。
...
- 未覆盖边界:
- 后续补跑计划:
- 实际用时 / 是否超时间盒:（调用方宿主时钟实测回填，见 [AI 协作规范 §1.3](../../../docs/standards/ai-collaboration.md)）
```
