...
3. **范围闸门**：发现新的优化点或非阻塞事项时，返回 P 阶段重新分流。

- **技能**：按需使用对应技术栈 skill（`nuxt`、`vue` 等）

### A (Audit) — 审计放行（强制）

1. **强制入口**：D 阶段完成后立即加载 `code-reviewer` skill 执行完整审查。
2. **审查范围**：代码、文档、配置、脚本均须审查。
3. **审计任务携带已查证事实**：把调研结论、实验证据、源码行号引用写进审计 prompt，避免审计者从头翻源码（证据优先级见 [AI 协作规范 §1.3 证据获取手段优先级](../../../docs/standards/ai-collaboration.md#证据获取手段优先级翻源码是最后手段)）。
4. **按风险分级分配深度**：高风险（发布/安全/配置/agent 定义）深度审计；低风险（文档/简单配置）快速审查，控制整体用时。
5. **退回策略**：发现 blocker 退回 D 或回流 P。

- **技能**：`code-reviewer`、`security-guardian`
...
- **技能**：`test-engineer`、`code-reviewer`

### F (Finish) — 文档收口与单次提交

1. **文档同步**：更新 `todo.md` 状态，按需同步相关文档。
2. **单次提交**：整个任务所有改动一次性提交。加载 `conventional-committer` skill，确认 A 阶段已放行且 lint/typecheck/测试通过，生成 Conventional Commits 消息并 `git commit`。
3. **推送禁令**：不自动 push。
