...
- [ ] Step 2: 逐类审计 ⚠️ REQUIRED
	...
	- [ ] 2.5 引入新依赖 / MCP server / 外部技能时，按 [安全规范 §5.2 供应链信任边界](../../../docs/standards/security.md) 核验来源与锁定（AI 推荐包幻觉、钉版本 + 锁文件、先验来源、依赖审计进 CI）。
- [ ] Step 3: 评估影响
...
- 信任 AI 推荐的包不查官方 registry（约 20% 不存在）或未钉版本直接安装。
- 引入外部 skill / agent / MCP 时不核对来源仓库与维护组织。
...
- [ ] 不确定的点已明确标注人工核验需求。
- [ ] 没有以环境或规模为理由弱化安全结论。
- [ ] 引入新依赖/工具/技能时，已核验来源与锁定（见 [安全规范 §5.2](../../../docs/standards/security.md)）。
