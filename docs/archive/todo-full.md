...
### Milestone 5: AI Breaking Change 研判

目标：引入 AI 能力，对依赖升级后的不兼容问题进行自动研判，生成修复方案并通过 PR 提交。

### Milestone 6: GitHub Action 增强 + 独立平台

目标：完善 GitHub Action 集成（自定义 AI Token + Prompt 注入防护），实现独立平台部署能力（权限控制、任务队列、批量处理）。
...
- 任务内容：
  - [x] 新增 CLI 入口目录。
  - [x] 新增配置层、GitHub 集成层、核心域层、修复器目录。
  - [x] 新增统一日志与错误模型基础设施。
- 完成定义：
  - [x] 项目结构与 [docs/plan.md](docs/plan.md) 的建议目录基本对齐。
...
- 任务内容：
  - [x] 定义运行模式、严重级别阈值、仓库列表、dry-run、是否创建 PR 等配置项。
  - [x] 支持环境变量与 CLI 参数合并。
  - [x] 给出默认值与校验错误提示。
- 完成定义：
  - [x] 本地运行时能解析一份完整配置。
...
## 4.6 Milestone 5: AI Breaking Change 研判
...
## 4.7 Milestone 6: GitHub Action 增强 + 独立平台

### T601 GitHub Action 用户自定义 AI Token 支持
...
- 任务内容：
  ...
  - [ ] AI 输入仅接受结构化数据（changelog、日志、diff），不接受自由文本。
  - [ ] 对 changelog 等外部输入做内容清洗（过滤 HTML/JS/Shell 注入标记）。
  - [ ] 系统 prompt 硬编码，与外部数据严格分离。
- 完成定义：
  - [ ] 非管理员无法触发 AI 分析。
  - [ ] 恶意内容无法注入到 AI prompt 中。
...
- 任务内容：
  - [ ] 平台 Web UI 框架搭建。
  - [ ] REST API 设计（仓库管理、任务创建、报告查看）。
  - [ ] 用户认证与 JWT 管理。
  - [ ] 数据库设计（仓库、任务、报告、用户）。
- 完成定义：
  - [ ] 可通过 Docker Compose 拉起平台基础服务。
...
- 交付物：支持连接多个 Git 平台仓库。
- 任务内容：
  - [ ] 支持 GitHub / GitLab / Bitbucket PAT 认证。
  - [ ] 仓库级别的配置管理（包管理器、忽略列表、自定义命令）。
  - [ ] 仓库连接状态监控。
- 完成定义：
  - [ ] 能通过 Web UI 添加和管理多个仓库连接。
...
6. T401 → T404，多仓库治理增强。
7. T501 → T504，AI Breaking Change 研判。
8. T601 → T602，GitHub Action AI Token + Prompt 注入防护。
9. T603 → T607，独立平台部署。
...
- [ ] GitHub Action 模式支持用户自定义 AI API Token。
- [ ] 具备 Prompt 注入防护，非管理员不可触发。
- [ ] AI 能对依赖升级 breaking change 做出研判并输出修复方案。
- [ ] 独立平台支持多仓库连接、任务队列和 RBAC 权限管理。
