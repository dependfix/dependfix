...
| G5 | M7 并发共享容器交叉污染 | P1 | ✅ **实施规划已就绪（2026-08-19）**——基于一手调研决策为 Docker rootless mode（runtime）+ 应用层白名单代理（网络）+ cgroup v2 + Node 20 自动识别（资源）。T1001-T1004 子任务已在 [todo.md §M10](../../plan/todo.md#m10-独立沙箱容器-c26-实施规划2026-08-19-启动) 拆解，设计在 [executor-sandbox.md §7](./executor-sandbox.md#7-sandbox-执行器设计) 落盘。`SandboxExecutor` 与 `ContainerExecutor` 并存（默认 `container`），K8s + Helm 仅规划登记 | [backlog C26](../../plan/backlog.md) |
...
- **C26/C40/C41 验收**：M10 实施规划已就绪（[todo.md §M10](../../plan/todo.md#m10-独立沙箱容器-c26-实施规划2026-08-19-启动)，2026-08-19），实现时逐项对照本文档第 4 节基线；网络白名单（T1002）与 cgroup 限制（T1003）有集成测试覆盖（含 docker rootless daemon 真实起容器实证）。C41 单命令超时已随 T802 落地（2026-08-14，进程树终止实证）。
