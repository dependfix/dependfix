...
| G5 | M7 并发共享容器交叉污染 | P1 | ✅ **M10 已闭环（2026-08-20 归档）**——基于一手调研决策为 Docker rootless mode（runtime）+ 应用层白名单代理（网络）+ cgroup v2 + Node 20 自动识别（资源）。T1001-T1004 子任务已在 [todo-archive.md §M10](../../plan/todo-archive.md#m10-独立沙箱容器-c26-实施规划已归档) 拆解并落地（13 commits 待推送），设计在 [executor-sandbox.md §7](./executor-sandbox.md#7-sandbox-执行器设计) 落盘。`SandboxExecutor` 与 `ContainerExecutor` 并存（默认 `container`），K8s + Helm 仅规划登记；**T1005 路由接线 4 子任务中 3 项已闭环**（2026-08-20：A 前端 UI 暴露 `0ea8149` / C 状态机扩 degraded `64135ed` / D quick-start.md 同步当前批次）；**B 仓库级 sandboxLimits 待启动**（单独批次，UI 不暴露限额覆盖表单）。详见 [executor-sandbox.md §7.8](./executor-sandbox.md) 降级状态机契约 + [backlog T1005](../../plan/backlog.md) |
...
- **C26/C40/C41 验收（✅ 已达成 2026-08-20 M10 收口）**：T1001 SandboxExecutor + DockerAdapter / T1002 出站白名单 deny-by-default 拦截代理 / T1003 cgroup v2 资源限制 / T1004 文档收口全部 commit + Review Gate Pass；详见 [todo-archive.md §M10](../../plan/todo-archive.md#m10-独立沙箱容器-c26-实施规划已归档)。T1002 与 T1003 单元 + 集成测试覆盖（含 docker rootless daemon 真实起容器实证，本机 WSL2 cgroup v1 环境 `describe.skipIf(!isCgroupV2)` 集成 stub）。C41 单命令超时已随 T802 落地（2026-08-14，进程树终止实证）。**剩余项**：T1005 sandbox 路由接线 4 子任务中 3 项已闭环（2026-08-20）——A 前端 UI 暴露 sandbox 选项（`0ea8149`）/ C 状态机扩 degraded（`64135ed`，见 [§7.8 契约](./executor-sandbox.md)）/ D 文档同步（当前批次）；**B Repository.sandboxLimits JSON 字段待启动**（单独批次，UI 不暴露限额覆盖表单）。详见 [backlog.md T1005](../../plan/backlog.md)。
