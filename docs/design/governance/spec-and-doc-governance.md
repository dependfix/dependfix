...
- **改动预计 > 5 文件 / > 350 行** → **建议有**专项设计文档；未提供时需在 commit message 显式声明"无专项设计 + 理由（用户已决策或 trivial 改动）"
- **A 阶段审计必查**：是否满足硬阈值 + 是否走对应 governance 文档（如未走，按 §6 触发 reject）

**示例**：

| 改动规模 | 设计文档要求 | 审计 depth |
|---|---|---|
| 1-5 文件 / < 350 行 | 无要求 | quick |
| 6-10 文件 / 351-800 行 | **建议**专项设计 | standard |
| > 10 文件 / > 800 行 | **必须**专项设计 + governance 落地 | deep |
| 跨 ≥ 2 个独立模块 | **必须**专项设计（不论规模）| standard 或 deep |
