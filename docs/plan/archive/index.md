...
## 4. 当前基线（2026-08-30 M18 阶段归档后）

- `roadmap.md`: 健康窗口（约 400 行，< 800 阈值）。M0-M18 全部归档；Milestone 概述表 19 阶段按时间顺序排列（M0→M18）；M18 行新增（已完成 2026-08-30 归档 / ~24 commits 已全部推送 ahead=0）；当前阶段任务指针更新（ahead=0 已全部推送 + 等待 M19）；M18 段新增详细实施状态段（与 M17 段风格对齐）。
- `todo.md`: 健康窗口（约 67 行，< 500 阈值）。M18 任务清单已迁出至 [todo-archive.md §M18](todo-archive.md#m18-平台-github-app-byo-app-模式m180m181m182m183m184m18x-全部已闭环--2026-08-30-归档)；顶部 banner 切换为"无活跃实施阶段（M18 已闭环归档 2026-08-30）— 等待 M19" + ahead=0（~24 commits 已全部推送至 origin/master）；保留待人工验收 3 项真实环境验证（T701/T702/T704）+ 发布管线收尾随可用性推进。
- `todo-archive.md`: **健康窗口（约 580 行，< 700 分片阈值）**。主窗口保留最近 5 个批次（回到"3-5 个阶段"健康策略）：**2026-08-30 M18 平台 GitHub App BYO App 模式（M18.0+M18.1+M18.2+M18.3+M18.4+M18.x 全部已闭环 / ~24 commits 已全部推送 ahead=0）/ 2026-08-28 M17 安全与可用性收口（M17.1+M17.2+M17.3+M17.4+M17.5+M17.6 全部已闭环 / 9 commits 已全部推送 ahead=0）/ 2026-08-28 M16 平台可用性深化（M16.1+M16.2+M16.3+M16.4+M16.5 已实施 / M16 全部闭环）/ 2026-08-26 M15 扫描历史详情侧栏增强（UX-R2）/ 2026-08-26 M14 platform release 通道闭环 + UX 反馈跟进（M14.1+M14.2+M14.3+M14.x+M14.y）** + M13/M12/M8/C53/M11 推进批次指针段。M18 段含 6 子阶段 + 1 治理批次闭环清单 + 验收 + 治理记录 + 关键决策 + 关键经验（集成外部库 README 标准用法 + 测试 stub 命名一致性）+ 待迁移经验（W1/W2/C34）。**本批次同期动作**：M13（26 commits / 4 子阶段 + T1310 同步）从 todo-archive.md 主窗口预防性迁出至新分片 [archive/todo-archive-phases-m13.md](archive/todo-archive-phases-m13.md)（M18 段新增前主窗口 673 行接近 700 分片阈值，预防性迁出与 M17/M16 批次同源策略）。
- `backlog.md`: 健康窗口（约 290 行，< 500 阈值）。本批次清理 1 个已闭环条目：**S-5 调用方测试 `process.env.ENCRYPTION_KEY` 死代码清理**（已由 M18.x 治理批次 commit `878ae1a` 闭环）—— 主条目从 §测试基础设施清理 段"已由 M18.x 闭环"标注。**同期更新**：M18 闭环整理记录追加到状态 banner（**2026-08-30 闭环整理（M18 归档批次）**）；M18 新增到 §已闭环阶段 段；M13 已闭环阶段引用更新为分片路径。**未删**：C34（存量规范严格约束挂接盘点）/ W1（stageAndCommit host 全局 config 隔离）/ W2（`detectServerLocale` 大小写敏感）——M18.x 剩余风险保留；已知边界 / 长期主线 / 周期性回归验证层 / C33 / C36 / C37 / D1-D8 / T701-e2e / SAML / B1-B2 / T905 / C22-C24 / G1 等不变。
- 分片记录：
