...
- `todo.md`: 健康窗口（已切换为 M16 P 阶段文档待用户指令进入 D 阶段）。M15 已全部闭环 + 3 commits ahead（`5c65177` P 阶段 docs + `1112017` feat 实施 + `0a60e3d` test 覆盖）按 [规划规范 §4.4 §5 ahead 实证](../../docs/standards/planning.md) `git rev-list HEAD ^origin/master --count` 动态核验（不写死具体数字以免 staleness）；M16 5 项原子任务见 `todo.md §M16 任务清单`；保留待人工验收 3 项真实环境验证（T701/T702/T704）+ 发布管线收尾随可用性推进。
