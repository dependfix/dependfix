...
- `todo.md`: 健康窗口（已切换为 M15 UX-R2 当前阶段）。M14 归档时 ahead=0（`git rev-list HEAD ^origin/master --count` 实证）；M14 归档完成后另有 `d517a7f` 发布链路提交，当前工作树相对 `origin/master` ahead=1。保留待人工验收 3 项真实环境验证（T701/T702/T704）+ 发布管线收尾随可用性推进；M15 只承接 UX-R2，UX-R3 顺延 M16。
