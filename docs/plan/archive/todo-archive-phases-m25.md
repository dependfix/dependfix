...
- **ahead commits 实证**：`git rev-list HEAD ^origin/master --count` = **17**（M25 阶段全部 17 commits，已推送 origin/master）
...
> - **ahead commits 实证**：`git rev-list HEAD ^origin/master --count` = **17**（已推送 origin/master）
