...
- **C81 源码 / 配置注释中的孤立规划编号清理（存量）** —— 2026-09-21 M29.9 A 阶段审计 B1 衍生；评估完成待上收；按 [规划规范 §3.1](../standards/planning.md#31-新需求默认走评估--backlog原则hard-requirement) **不带 M\d+ 阶段编号**。
  ...
  - **现状实证**（2026-09-21 启发式扫描；**量级估算**，精确值随扫描范围浮动）：
    - **判定口径**：扫描「非 `docs/` 的源码 / 配置 / 脚本」（skip：`node_modules` / `dist` / `.nuxt` / `pnpm-lock`），行级判定「同行是否含 `docs/` / `.md` / `§` / `todo.md` 等文档指针」——带指针者为合规例外，无指针者为孤立疑似违规。
    - **实测（行口径，自洽闭合）**：扫描 1023 文件 → 命中行 **807** ＝ 带文档指针 **380** + 孤立疑似违规 **427**（同行含多个编号时按行计一次）。审计方以更窄 skip 集（582 文件）独立扫描得孤立 **310** 行。
    - **量级结论**：孤立命中约在 **300-430 行**区间浮动（差异源于扫描范围与判定粒度）；**任一量级均远超 [§1.1 任务粒度约束](../standards/planning.md#11-硬性约束) 单批阈值**，故「必须分批」的结论不依赖精确值。本条目不复述单一精确数字——上收首步即产出可复现的检测脚本并固化口径（见决策点与验收标准）。
