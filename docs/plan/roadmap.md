...
| M14: platform 进入 release 通道 + UX 反馈跟进 | 让 `apps/platform` 作为第 6 个发布单元参与 release 链路但**不发 npm**——仿 momei 单包"独立 version + 独立 CHANGELOG"的精神，适配 dependfix monorepo + docker-only 平台 + 承接 backlog UX-R1 扫描历史分页（用户实测反馈痛点）：① `scripts/packages.config.mjs` 注册 apps/platform 条目（`npmPublishable:false`）；② `scripts/release-publish.mjs` 新增 tag-only action；③ `docker.yml` 支持 workflow inputs 读 platform_version；④ `release.yml` 完成后触发 docker workflow_dispatch；⑤ `docs/guide/release.md` 平台独立通道文档；⑥ dependabot 排除 `apps/platform/package.json`；⑦ `/api/runs` 新增 `page`/`pageSize`/`ids` 分页参数 + `{items, total, page, pageSize}` 返回结构；09 4 个前端调用方适配（RepoHistoryDialog PrimeVue Paginator + alerts.vue + repos/[id]/runs.vue + i18n）+ silent bug 修复（alerts sidebar ids 参数） | P1 | 部分完成（M14.1 T1310 F 阶段闭环 ✅ 2026-08-26 落地 7 commits / M14.2 UX-R1 扫描历史分页 ✅ 2026-08-26 落地 4 atomic commits 后端分页 + RepoHistoryDialog Paginator + 次级调用方 + i18n + e2e + 收口登记；ahead commits 待用户推送 `git rev-list HEAD ^origin/master --count` 实证核验；M14.3 M13.4 T1403 follow-up + M14.x neat-freak 批次 2 子阶段计划中；详见 [todo.md §M14](todo.md#当前阶段m14-platform-release-通道闭环--ux-反馈跟进) + [todo-archive.md §M14.1/2](todo-archive.md#m14-platform-release-通道闭环--ux-反馈跟进m1412-已闭环--m143x-计划中)） |
...
- 当前阶段任务：[todo.md](todo.md)（**M14 platform 进入 release 通道 + UX 反馈跟进**，2026-08-26 启动；M14.1 T1310 F 阶段闭环 ✅ + M14.2 UX-R1 扫描历史分页 ✅ 已落地；M14.3 M13.4 T1403 follow-up + M14.x neat-freak 批次 2 子阶段计划中；ahead commits 待用户推送 `git rev-list HEAD ^origin/master --count` 实证核验；待人工验收 T701/T702/T704 项随真实环境推进）
