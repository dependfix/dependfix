...
| M24: PR Check MVP + 治理债 + 测试补强 + 用户体验 | 2026-09-03 用户决策启动方案 B（能力突破优先），按"类型平衡"原则（🚀 能力 1 + 🛡️ 治理 2 + 🧪 测试 1 + 🎨 体验 1）拆 **5 原子条目独立闭环**：M24.1（P1，🚀 能力）PR Check 状态监测 MVP（~1100 行新建 + ~70 行修改 + 5 phase 串行；P 阶段决策 D1-D8 全部 2026-09-02 用户决策落地）/ M24.2（P2，🛡️ 治理）M22.7+M22.8 根因 4 项残留源码排查（候选 ③/① 已治本；剩 ①/②/④ 仅做源码层面排查，不依赖非 sandbox 环境 CI 复现）/ M24.3（P3，🧪 测试）cron-preview wall-clock 依赖消除（M23.4 commit `df4ba9b` 后续 audit suggest 1+2+3 闭环）/ M24.4（P3，🛡️ 治理）M18.x W1+W2 + Code Scanning RG-W01/W02 集中清理（~110 行 + 3 测）/ M24.5（P2，🎨 体验）C36 服务端 API i18n 扩展（沿用 M17 已沉淀 createLocalizedError 模式 + 选定 pr-checks 模块与 M24.1 同步实施避免硬编码英文） | P1-P3 | **已完成**（2026-09-03 完整闭环 5 原子条目 12 commits / ~2960 行净增；详见 [todo-archive.md §M24](todo-archive.md#m24-pr-check-mvp--治理债--测试补强--用户体验m241m242m243m244m245-全部已闭环--2026-09-03-归档) + [archive/todo-archive-phases-m24.md 完整实施记录](archive/todo-archive-phases-m24.md)） |
| M25: PrimeUI License 治理 + 平台 AI 研判集成 + lint baseline 治理 + M24 follow-up 工具化 | 2026-09-08 用户决策启动方案 A（治理优先 + 能力扩展 + 测试补强），按"类型平衡"原则（🛡️ 治理 2 + 🚀 能力 1 + 🧪 测试 1）拆 **4 原子条目独立闭环**：M25.1（P1，🛡️ 治理）C70 PrimeUI 主题库降级（`@primeuix/themes` 3.x → 2.x，PrimeUI License → MIT 化，2-3 commits / 30-60 min）/ M25.2a（P1，🚀 能力）C68 平台 AI 研判集成「基础层」（数据模型 Organization/Repository/ScanRun 实体 + ScanRequest schema 扩展 + scan-orchestrator service 透传 + 三执行器 container/sandbox/github-action 同步透传 + RuntimeConfig.ai 工具抽取，3-4 commits / ~810 行；M25.2b 应用层 API+UI+e2e+docs 5 commits / ~1130 行 推 M26 阶段，按 [规划规范 §1.1 阶段性聚焦 5-6 项以内](https://github.com/CaoMeiYouRen/dependfix/blob/master/docs/standards/planning.md) + §1.1 任务粒度约束跨模块骨架任务 > 5 文件即考虑拆分）/ M25.3（P3，🛡️ 治理）apps/platform baseline 16 lint errors 清理（删除 16 处 bare expressions 占位符——`no-unused-expressions` 或 `no-meaningless-void-operator` 触发，1 commit / ~-16 行）/ M25.4（P3，🛡️🧪 治理 + 测试）M24 follow-up 工具化（i18n-anchor-check 脚本 + zod-helpers parseOptional helper + 替换经验归档 §五十六 Phase 3 W2 + Phase 2 W6，2 commits / ~330 行） | P1-P3 | **已完成**（2026-09-08 完整闭环 4 原子条目 17 commits / ~1821 行净增；ahead=17 待用户主动推送；详见 [todo-archive.md §M25](todo-archive.md#m25-primeui-license-治理--平台-ai-研判集成--lint-baseline-治理--m24-follow-up-工具化m251m252am253m254-全部已闭环--2026-09-08-归档) + [archive/todo-archive-phases-m25.md 完整实施记录](archive/todo-archive-phases-m25.md)） |
...
## M23: M22 治理债收口 + 根因排查 + 能力扩展 + 测试补强（已完成 2026-09-02 归档）
...
**状态**：✅ 全部完成 5 原子条目（M23.0 + M23.1 + M23.2 + M23.3 + M23.4）共 **17 atomic commits 全部 ahead=0 已推送至 origin/master**（2026-09-02 实测 `git rev-list HEAD ^origin/master --count` = 0）。详见 [todo-archive.md §M23](todo-archive.md#m23-m22-治理债收口--根因排查--能力扩展--测试补强m230m231m232m233m234-全部已闭环--2026-09-02-归档) 阶段闭环清单 + 阶段治理记录。

### M23 阶段交付概览（已闭环）

- **总条目**：5 原子条目（M23.0 治理批次 / M23.1 M22.7 根因排查 / M23.2 M22.8 根因排查 / M23.3 C66 告警视图增强 / M23.4 测试补强）
- **类型分布**：🛡️ 治理 1 + 🛡️ 治理/治本 2 + 🚀 能力扩展 1 + 🧪 测试补强 1
- **实际 commits**：**17 atomic commits**（M23.0 G1+G2+G3 + commit hash 回填 4 / M23.1 3 / M23.2 4 / M23.3 5 / M23.4 1）—— 超出预估 8-12 范围（M23.3 C66-C standard depth audit + 多次 commit hash 回填导致 commits 偏多）
- **审计覆盖**：5 轮独立 Review Gate（M23.3 C66-C standard depth Round 1 + M23.0/M23.1/M23.2/M23.4 quick depth）全部 Pass；含 C66-C standard depth W1 typecheck 验证矩阵不完整（git stash 实证非本批引入）+ W2 浏览器验证 sandbox chromium 限制 + W3 todo.md stale 已本批同步修正 + W4 i18n 9 语言声明错引已本批同步改为双语言现状
- **关键经验**（详见 [todo-archive.md §M23 阶段治理记录](todo-archive.md#m23-m22-治理债收口--根因排查--能力扩展--测试补强m230m231m232m233m234-全部已闭环--2026-09-02-归档)）：
  - W1 audit typecheck 验证矩阵不完整——`pnpm exec tsc --noEmit` 通过 ≠ `pnpm run typecheck`（含 nuxt typecheck pipeline）通过；monorepo source-only 改动必须 `pnpm -r build` 重建 dist（已挂 [AGENTS.md 提交规范第 6 条「build-before-commit」纪律](../../AGENTS.md)）
  - W3 todo.md 验收清单 stale——D 阶段开工前 `rg` 实证依赖项实际状态（git 历史 + i18n locale 目录）避免基于 stale 描述定范围
  - W4 i18n locale 声明与现状不符——`ls apps/platform/i18n/locales/` 实证实际 locale 数量（已挂 [code-auditor.agent.md 主责边界「i18n locale 实际状态审计必查项」](../../.github/agents/code-auditor.agent.md)）
- **M23 阶段归档批次衍生治理**（本归档批次落地 3 项 governance check point）：AGENTS.md 提交规范第 6 条「build-before-commit」+ code-auditor 主责边界「i18n locale 实际状态审计必查项」+ wisdom.md monorepo rebuild 教训合并到现有 `principle-Nitro-esbuild` 段

---

## M24: PR Check MVP + 治理债 + 测试补强 + 用户体验

> ✅ **完整闭环** 2026-09-03（方案 B 能力突破优先，5 原子条目 12 commits / ~2960 行净增）。

承接 M23 闭环后 backlog 候选池，2026-09-03 用户决策启动方案 B（能力突破优先）。**方案 B 关键决策**：① 方案选择 = 方案 B（PR Check MVP 单条目占 M24 总规模 ~58% + 配套治理 + 测试补强 + 体验扩展）；② M24.2 候选替换 = M22.7+M22.8 根因 4 项残留源码排查（**原方案 B 的 M24.2「M22 neat-freak 收敛」已被 M23.0 G1 commit `f8a8640` docs(standards) 闭环**，security.md §2.1 为 SQLite 防护规则权威完整声明，development.md §5.1.18 + platform.md §3.7 第 1/2/3 条收敛为引用 + 仅保留差异化信息）；③ M24.5 体验扩展 = C36 服务端 API i18n（而非 UX-R3 `/scans` 页面深化），且选定 **pr-checks 模块**与 M24.1 同步实施（避免 pr-checks API 错误消息硬编码英文）；④ PR Check MVP 保持在 M24.1 内（不独立 M25 阶段；5 phase 串行严格按 M24.1.1-5 顺序）；⑤ wisdom 蒸馏：M24 阶段累积 8 条新 pattern 已在 commit `7c926a9` 蒸馏批次挂接 standards（4 文件 75 行净增）。

**状态**：✅ 已完整闭环 2026-09-03（ahead=3 累计 commits `bbb8f30` / `ad1ab17` / `a0be125` / `aaf8e7b` / `7c926a9` 待用户主动推送；前 4 个 commits 已合并至 origin/master）；详见 [todo.md §M24 完整闭环](todo.md) + [todo-archive.md §M24 导航指针](todo-archive.md#m24-pr-check-mvp--治理债--测试补强--用户体验m241m242m243m244m245-全部已闭环--2026-09-03-归档) + [archive/todo-archive-phases-m24.md 完整实施记录](archive/todo-archive-phases-m24.md) + 关键经验 + 待迁移经验。

### M24 阶段交付概览（已闭环）

- **总条目**：5 原子条目（M24.1 PR Check MVP / M24.2 M22.7+M22.8 根因排查 / M24.3 cron-preview / M24.4 M18.x+Code Scanning / M24.5 C36 i18n）全部 [x]
- **类型分布**：🚀 能力 1 + 🛡️ 治理 2 + 🧪 测试 1 + 🎨 体验 1
- **实际规模**：~2960 行净增（实测：PRCheck 实体 230 + service 940 + API 730 + UI 490 + UI follow-up 30 + UI 重构 90 + Phase 5 docs 127 + M24.2 docs 195 + CI 修复 5 + M24.3 docs 25 + M24.4 docs 25 + Wisdom 蒸馏 75）+ 12 atomic commits
- **P 阶段决策（已落地）**：
  - M24.1 PR Check D1-D8 全部 2026-09-02 用户决策落地：PRCheck 实体独立于 ScanResult（`apps/platform/server/entities/pr-check.ts` 新建）+ Polling 间隔 5min/仓 + 失败 PR firing alert + ack UI + 用户手动创建 schedule 启用 + webhook MVP 仅接口预留 + 仅 per-org scope + env 开关 `ACTION_STATUS_MONITOR_ENABLED` 默认 false + 文档明确 mergify 仍是主控（D8 跨文档分散：dependfix README + `.github/mergify.yml` 注释 + PRCheck 设计文档 + experience-archive §五十六）
  - M24.2 根因 4 项残留候选最终状态：① better-auth transaction 已治本（typeorm-adapter L237-241 走真事务；better-auth 1.7.2 `adapter-base.mjs:18` 自动 patch fallback 不适用）+ ② Nitro h3 async generator 非根因（fixtures 均为 `async (event) => {}` 非 generator）+ ③ SQLite WAL M23.1 已闭环 + ④ fixtures 节流经验性方案登记 follow-up
- **关键风险最终状态**：
  - **R1** ✅ PRCheck 复合唯一索引声明正确（§3b 教训 + SQLite DDL 实证：6 索引全部生成正确列结构）
  - **R2** ✅ mergify 仍按 `check-success=Test` 单条件触发；PRCheck 监测失败不阻塞 mergify（D8 跨文档三处同步）
  - **R3** ✅ TypeORM synchronize / migrationsRun 默认值符合 §5.1.19 硬要求（PRCheck entity migration `1800000000000` 手动执行）
  - **R4** ✅ CLI 端 entity metadata 显式 import 落地（PRCheck entity 已注册到 `apps/platform/server/database/index.ts`）
  - **R5** ✅ M24.2 根因 4 项本地源码排查完成（3 候选已治本 + 1 候选 follow-up 登记）
  - **R6** ✅ C36 i18n 范围控制（M24.5 单条目 ≤ 4 端点 = 实际 4 端点 + 3 错误码 + 17 单测合 1 commit `89e1344`）
...
- 当前阶段任务：[todo.md](todo.md)（**M24 阶段 2026-09-03 用户决策启动**——方案 B 能力突破优先，5 原子条目独立闭环：M24.1 PR Check MVP / M24.2 M22.7+M22.8 根因排查 / M24.3 cron-preview wall-clock / M24.4 M18.x+Code Scanning / M24.5 C36 i18n；详见 [todo-archive.md §M24](todo-archive.md#m24-pr-check-mvp--治理债--测试补强--用户体验m241m242m243m244m245-全部已闭环--2026-09-03-归档)；待人工验收 T701/T702/T704 项随真实环境推进）
