...
- **发布管线自研化（进行中，2026-08-10）**：移除 changeset，自研 release 脚本体系（双模式：A 本地手动提升 + B CI 定时自动，参照 semantic-release）。设计文档 [release-pipeline.md](../design/governance/release-pipeline.md) 落盘；拆分 4 提交（release-version 执行器 → release-publish 发布器 → 原子切换 → 文档收口）。**进度**：提交 1（`scripts/release-version.mjs` 版本提升执行器 + 16 单测，依赖传导替代 `updateInternalDependencies`）已完成并 Review Gate Pass（P3 观察项：main 副作用路径测试登记后续）。待提交 2-4。
- **已知边界**：
