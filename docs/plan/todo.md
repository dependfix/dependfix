...
> 7. **Card+DataTable 空状态双线 UX bug**（2026-09-03 治本修复：`repos.vue` 等 Card 包 DataTable 页面空状态时表头下方出现"两条横线" — `.p-datatable-thead th` border-bottom 与 `.p-datatable-tbody > tr.p-datatable-empty-message > td` border-bottom 中间隔 5px 空白，视觉上形成"双线"。**根因**：PrimeVue Aura 主题默认给每行 TD 1px slate-200 border-bottom（有数据时是"行间分隔"），但空状态无数据时 TD border 失去"行间分隔"语义，仅成为"无意义第二条线"。**治本方案**（`apps/platform/app/assets/styles/main.scss` 加 1 段 ~5 行 SCSS）：仅空状态行去除 TD border-bottom — 语义正确（无数据无需行间分隔）+ 作用域最小（不影响有数据状态）+ 全局生效（repos / scans / alerts-rowgroup / pr-checks 同步修复）+ light/dark mode 都覆盖（同一规则、变量驱动颜色）。A 阶段 quick depth 审计。单 atomic commit，关联本次 follow-up 条目）
>
...
- [x] follow-up 登记（如本批次无法本地验证，登记 follow-up 候选）
- [x] 编号标记扫描 0 命中
...
| 阶段 | Commit | 范围 |
|:---|:---|:---|
| 源码追溯 | `<commit>` | 3 份源码追溯报告 + experience-archive §五十七 + todo.md §M24.2 验收清单 + follow-up 登记 |
