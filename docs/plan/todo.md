...
- **实现内容**：
  - [ ] 子任务 1（i18n 基建）：
    - [ ] 依赖：`@nuxtjs/i18n`（apps/platform）+ nuxt.config `i18n` 配置（locales/strategy/defaultLocale/detectBrowserLanguage 关闭由 detector 接管）
    - [ ] `app/i18n/localeDetector.ts`：URL > Cookie > Accept-Language > zh-CN 检测纯函数
    - [ ] `app/i18n/locales/zh-CN.json` + `en-US.json`：common 命名空间骨架（导航/通用按钮/对话框/空态/错误提示）
    - [ ] 导航栏语言切换器（default.vue）+ settings.vue 语言偏好激活（占位 Select 替换为真实切换，写 cookie + setLocale）
    - [ ] PrimeVue locale 联动插件（locale 变化 → `usePrimeVue().config.locale` 切换 zh_CN/en）
    - [ ] vueI18n datetime/number formats 配置（zh-CN / en-US）
  - [ ] 子任务 2（认证与框架文案抽取）：
    - [ ] login.vue / register.vue（登录注册 + OAuth/OIDC 按钮 + 注册准入提示）
    - [ ] settings.vue / users.vue（个人设置 + 用户管理，含语言偏好文案）
    - [ ] default.vue 导航 / app.vue / index.vue / dashboard.vue（框架 + 仪表板）
  - [ ] 子任务 3（业务大页文案抽取）：
    - [ ] repos.vue（仓库列表/导入/编辑/批量扫描弹窗，含 tags 标签输入）
    - [ ] schedules.vue（定时计划列表/编辑/触发/4 策略动态表单）
  - [ ] 子任务 4（其余业务 + 收尾）：
    - [ ] alerts.vue / credentials.vue / batch-runs.vue / repos/[id]/runs.vue 文案抽取
    - [ ] 硬编码文案零命中扫描（模板 + script 用户可见字符串 grep 中文零命中，排除注释/console/测试）
    - [ ] e2e：语言切换闭环（切 en-US 断言英文文案 + PrimeVue 对话框按钮 + 回切 zh-CN 断言中文）+ 全量回归
    - [ ] 文档收口：README / 部署文档补 i18n 说明
...
- 完成定义：
  - [ ] 切换语言后全平台页面文案切换（zh-CN / en-US），无硬编码文案残留（grep 扫描零命中）
  - [ ] 默认 zh-CN 无前缀 URL 行为正确，`/en` 前缀路由可用
  - [ ] PrimeVue 组件内置文案（对话框按钮/表格空态）随语言切换
- 验收：
  - [ ] 语言检测：URL > Cookie > 浏览器 > 默认优先级（单测 localeDetector 纯函数）
  - [ ] 语言切换器：导航栏 + settings.vue 双入口联动（setLocale + cookie 持久化，刷新保持）
  - [ ] PrimeVue locale 联动：切换后对话框按钮/空态文案变化（e2e 断言）
  - [ ] 时间/日期/数字格式：随 locale 变化（页面断言）
  - [ ] 硬编码扫描：grep 中文零命中（模板 + script 用户可见字符串）
  - [ ] e2e：i18n 切换闭环用例 + 全量回归（基线 25+）
  - [ ] 单测 + lint/typecheck/build 全过
