...
- **审计调用协议**：`Full Stack Master (全栈大师)` 发起审计时必须显式声明 `audit-depth`（quick / standard / deep + 理由）、变更文件清单、已验证证据摘要与复审问题编号，并在审计 prompt 中携带**审计启动时间戳**（宿主系统时钟，格式如 `2026-08-13T14:30:00+08:00`）；未声明按 `deep` 防御执行；
- **真实用时实测（时间盒核验依据）**：LLM 自报用时是估算值，无真实时间感知，**不得作为时间盒核验依据**。时间盒核验由调用方实测：发起审计 task 前用宿主系统时钟记录启动时间戳（PowerShell 用 `Get-Date -Format o` 等），审计返回后用系统时钟计算 elapsed，把"实际用时 / 是否超时间盒"回填审计结论与证据记录；实测超时（elapsed > 时间盒）时结论标注"超时"，并要求审计方补充未覆盖边界。审计方需判断是否接近时间盒上限时，可用 bash 执行时间命令对比 prompt 中的启动时间戳（秒级成本，仅收敛判断用，不回填结论）；
