...
6. **批量替换纪律**：脚本/正则批量改写代码时，先改 1 个代表性文件 → typecheck + diff 审查 → 确认无误再铺开全量；正则必须限定上下文（注释行、字符串前缀、精确清单），禁止 `[^)]*`、`.*?` 等通配在注释与代码混合文件中跨上下文匹配；写文件必须按行保留原行尾（混合行尾仓库整体转换会制造全文件噪音 diff）；替换后验证矩阵 = typecheck + 定向测试 + `git diff --ignore-space-at-eol` 噪音检查 + 残留扫描，涉及外链文本时额外核对（check-links 只查本地链接）。教训见 [经验归档 §十七](../design/governance/2026-08-06-experience-archive.md)。
