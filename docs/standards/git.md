...
5. **分批提交（长任务强制）**：单次提交建议规模 ≤ 10 个文件且 ≤ 800 行新增；超出时先在 P 阶段把任务拆分为多个原子条目，按"可独立验证"的顺序分批次提交（每批独立过 Review Gate），禁止把整条长任务攒成一次巨型提交（教训见 [经验归档 §二十四](../design/governance/2026-08-06-experience-archive.md)：T601 单次 40 文件/+8329 行导致审计 5 轮、修复往返 10+ 处）。锁文件（pnpm-lock.yaml）等随其所属批次提交。
