...
- **不能用 `process.env.NODE_ENV === 'production'` 作第二门控**（Nitro/esbuild 构建期把 `process.env.NODE_ENV` 静态替换为构建时值，prod build 表达式折叠后永远 404；详见 [platform.md §3.6](./platform.md) 陷阱段）
